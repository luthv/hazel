...
module.exports = class Cache {
  constructor(config) {
    ...
    this.cacheYml = this.cacheYml.bind(this)
  ...
  async cacheYml(id) {
    const { token, account, repository } = this.config
    const headers = { Accept: 'application/octet-stream' }

    const url = `https://${token}:@api.github.com/repos/${account}/${repository}/releases/assets/${id}`;
    
    const { status, body } = await fetch(url, { headers })

    if (status !== 200) {
      throw new Error(
        `Tried to cache latest.yml, but failed fetching ${url}, status ${status}`
      )
    }
    ...
    for (const asset of release.assets) {
      ...
      if (name === 'latest.yml') {
        try {
          ...
          this.latest.files[name] = await this.cacheYml(id)
        } catch (err) {
          console.error(err)
        }
        continue
